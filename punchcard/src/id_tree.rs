...
impl<T> Arena<T> {
    ...
    /// Remove a node and its children from the arena.
    pub fn remove_node(&mut self, id: NodeId) -> T {
        use std::mem::replace;

        id.detach(self);
        // list of IDs to remove
        let removed = id.descendants(self).collect::<Vec<_>>();
        // drop children
        for id in removed.iter() {
            // drop node
            replace(&mut self.nodes[id.index].data, None);
        }
        // add IDs to free list
        self.free_list.extend(removed);
        // remove node
        let data = replace(&mut self.nodes[id.index].data, None).unwrap();
...
impl<T> Node<T> {
    ...
    /// Returns a reference to the node data. Panics if the NodeId is invalid.
    pub fn data(&self) -> &T { self.data.as_ref().unwrap() }

    /// Returns a mutable reference to the node data. Panics if the NodeId is invalid.
    pub fn data_mut(&mut self) -> &mut T { self.data.as_mut().unwrap() }
}
