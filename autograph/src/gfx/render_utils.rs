...
pub trait DrawUtilsExt<'queue>
{
    ...
    fn blit_texture(&self, target: &Framebuffer, tex: &gfx::Texture2D, sampler: &gfx::SamplerDesc, rect_transform: &RectTransform);
}
...
impl<'queue> DrawUtilsExt<'queue> for Frame<'queue>
{
    ...
    fn blit_texture(&self, target: &Framebuffer, tex: &gfx::Texture2D, sampler: &gfx::SamplerDesc, rect_transform: &RectTransform)
    {
        ...
        let calc_transform = rect_transform.calculate_in_parent(&na::Matrix3::identity(), &na::Vector2::new(fw,fh));
        let ndc_transform = na::Matrix3::new(2.0, 0.0, -1.0, 0.0, 2.0, -1.0, 0.0, 0.0, 1.0) * calc_transform.transform;
