...
#[derive(Debug,Clone)]
pub struct ITypeVoid { pub result_id: u32 }

#[derive(Debug,Clone)]
pub struct ITypeBool { pub result_id: u32 }
...
fn decode_instruction(opcode: u16, operands: &[u32]) -> Result<Instruction, ParseError>
{
    ...
    Ok(match opcode {
           ...
           14 => Instruction::MemoryModel(try_parse_constant::<AddressingModel>(operands[0])?,
           ...
           16 => {
               Instruction::ExecutionMode(IExecutionMode {
                   target_id: operands[0],
                   mode: try_parse_constant::<ExecutionMode>(operands[1])?,
                   optional_literals: operands[2..].to_vec(),
               })
           }
           17 => Instruction::Capability(try_parse_constant::<Capability>(operands[0])?),
           19 => Instruction::TypeVoid(ITypeVoid { result_id: operands[0] }),
           20 => Instruction::TypeBool(ITypeBool { result_id: operands[0] }),
           ...
           26 => Instruction::TypeSampler(ITypeSampler { result_id: operands[0] }),
           ...
           248 => Instruction::Label(ILabel { result_id: operands[0] }),
           249 => Instruction::Branch(IBranch { result_id: operands[0] }),
