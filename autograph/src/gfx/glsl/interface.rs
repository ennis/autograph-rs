        ...
        for inst in self.0.instructions.iter() {
            ...
            match *sr {
                ShaderResource::UniformConstant(desc) => {
                    ...
                    let location = desc.index.expect("must provide a location for uniform constants");
                    ...
                    if shader_loc != location { continue }
                    self.compare_types(resource_ty_inst, u.ty)?;
                    return Ok(true)
                },
                ShaderResource::UniformBuffer(desc) => {
                    // Storage class is uniform, and has a Block decoration
                    let binding = u.index.expect("must provide a buffer binding index for uniform buffers");
                    ...
                    let shader_binding = self.find_binding_decoration(var.result_id).expect("uniform buffer with no binding");
                    if shader_binding != binding { continue }
                    unimplemented!()
                },
                ShaderResource::ShaderStorageBuffer(desc) => {
                    ...
                    let binding = u.index.expect("must provide a buffer binding index for shader storage buffers");
                    ...
                    let shader_binding = self.find_binding_decoration(var.result_id).expect("SSBO with no binding");
                    if shader_binding != binding { continue }
                    unimplemented!()
                },
...
/// The result of looking for a shader resource in the shaders.
enum VerifyResult {
    /// No matching shader resource was found. It might have been optimized away.
    NotFound,
    /// A matching shader resource was found.
    Match,
    /// A shader resource with the correct location or name was found, but the type or layout does not match.
    Mismatch(Error)
}
...
impl SpirvGraphicsPipelineModules {
    ...
    fn verify_shader_resource<'a>(&self, sr: &ShaderResource<'a>) -> Result<(),Error>
    {
        ...
        let found_in_fs = self.gs.verify_shader_resource(sr)?;
        ...
        if !found {
            warn!("Could not find a corresponding shader resource for {:?}", sr);
        }

        Ok(())
    }

    fn verify(&self, interface: &ShaderInterfaceDesc)
    {
